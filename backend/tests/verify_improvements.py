...
def test_script_functions():
    ...
    try:
        from app.scripts.pdf_to_doc import (
            pdf_to_doc_pdfminer,
            pdf_to_doc_fitz,
...
def test_ppt_detection_logic():
    ...
    print(f"A4 文档     - 宽高比: {aspect_a4:.3f}, 检测: {'❌ 普通PDF' if not is_ppt_a4 else '✅ PPT'}")
...
def test_file_size_threshold():
    ...
    print(f"小文件 (<20MB): 使用 pdfminer (稳定性优先)")
    print(f"大文件 (≥20MB): 使用 PyMuPDF (性能优先)")
